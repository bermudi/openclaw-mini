...
export function buildInitialPlan(discovery: SetupDiscovery): SetupPlan {
  return {
    ...
    telegramBotToken: discovery.existingChannels?.telegram?.botToken ?? '',
    telegramWebhookSecret: discovery.existingChannels?.telegram?.webhookSecret ?? '',
    telegramTransport: discovery.existingChannels?.telegram?.transport ?? 'webhook',
