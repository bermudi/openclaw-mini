...
The system SHALL include a `skills/researcher/SKILL.md` file with frontmatter fields: `name: researcher`, `description` explaining web research and summarization capability, and `tools` listing `web_search`, `web_fetch`, and `write_note`.

#### Scenario: Skill discovered and loaded
- **WHEN** the skill-service scans the skills directory
- **THEN** a skill named `researcher` SHALL be loaded with `enabled: true` (assuming tools exist)
...
The SKILL.md body SHALL contain substantive instructions (minimum 30 lines) covering: role definition, search strategy (multiple queries, cross-referencing), source evaluation, summarization format, tool usage patterns for `web_search` and `web_fetch`, output format (structured findings), and error handling (search failures, unreachable URLs).
...
- **THEN** `overrides.systemPrompt` SHALL NOT be set — the body is the canonical system prompt
