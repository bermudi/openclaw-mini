...
The system SHALL allow each sub-agent definition to include an `overrides` object specifying zero or more of the following fields: `model`, `provider`, `credentialRef`, `systemPrompt`, `maxIterations`, `allowedSkills`, `allowedTools`, `maxToolInvocations`. At least one field SHALL be present and all values SHALL pass schema validation (e.g., provider exists in registry, positive integers, referenced skills/tools exist).
