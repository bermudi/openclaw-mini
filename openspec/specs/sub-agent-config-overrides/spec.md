...
The system SHALL allow each sub-agent definition to include an `overrides` object specifying zero or more of the following fields: `model`, `provider`, `credentialRef`, `systemPrompt`, `maxIterations`, `allowedSkills`, `allowedTools`, `maxToolInvocations`. At least one field SHALL be present and all values SHALL pass schema validation (e.g., known providers, positive integers, referenced skills/tools exist).

#### Scenario: Valid override payload
- **WHEN** a sub-agent manifest contains an overrides object with `model: "openrouter/gpt-4"` and `maxIterations: 8`
- **THEN** config validation SHALL accept the manifest and record the overrides for runtime use.

#### Scenario: Invalid override payload
- **WHEN** a sub-agent manifest provides an overrides object with `provider: "made-up"`
- **THEN** config validation SHALL reject the manifest with an error indicating the provider is unsupported.

### Requirement: Deterministic override merge
The runtime SHALL compute the effective configuration for each sub-agent by merging base gateway defaults, agent profile values, and the declared overrides in that order, with override fields replacing inherited values. Only fields specified in overrides SHALL change; unspecified fields SHALL remain from upstream layers.

#### Scenario: Override replaces model only
- **WHEN** a sub-agent override sets `model: "anthropic/sonnet"`
- **THEN** the resolved config SHALL use the new model while retaining the parent provider, prompt, and limits.
